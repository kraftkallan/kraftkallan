 ...
 ************/
     ...
     */
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			$indent = str_repeat( "\t", $depth );
				$output .= $indent."<div class=\"sk-navbar-dropdown sk-is-boxed";
    ...
    /**
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$class_names = str_replace(array($fa,$show_title_class,"sk-is-right"),"",$class_names);
			$classes = empty( $item->classes ) ? array() : (array) $item->classes;
			$this->right_class = $classes;

			if ( ! $args['has_children'] ) {//if doesn't have children
						if ( ! in_array( 'sk-navbar-divider', $classes ) ) {//if doesnt contains a navbar divider
						...
						$item_output = '<hr class="sk-navbar-divider">';//output navbar divider
					}
				...
				$item_output .= $indent . '<div class="sk-navbar-item sk-has-dropdown sk-is-hoverable" data-target="dropdown"><!-- START DROPDOWN-->' . "\n";
    ...
    /**
		...
		private function get_item_classes( $item, $args ) {
			...
			$classes[]   = 'sk-navbar-item';
			$class_names = join( ' ', apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item, $args ) );
			if ( $args['has_children'] ) {
				$class_names .= ' dropdown sk-navbar-link';
			}
			if ( in_array( 'current-menu-item', $classes, true ) ) {
				$class_names .= ' sk-is-active';
			}
     ...
     */
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				echo '>';
				echo '<a class="sk-navbar-item" href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_attr( 'Add a menu', 'worldoweb' ) . '</a>';
				echo '</div>';
				if ( $container ) {
					echo '</' . esc_attr( $container ) . '>';
				}
